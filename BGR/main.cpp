...
*/
		...
		EPCIO_DAC_SetOutput(CHANNEL_INDEX1, 0, CARD_INDEX1);
		EPCIO_DAC_SetOutput(CHANNEL_INDEX2, 0, CARD_INDEX1);
...
void TSS_search(Mat input_background ,Mat input_target)
{
	...
	double target_total = input_target.rows *input_target.cols;
	double background_total  = input_background.rows *input_background.cols;
	double R1 = sqrt(target_total/background_total);
	double R2 = 1/R1;
	double xx2 =0;
			...
			for(int i=0;i<9;i++){
				...
				for(int k=(-input_target.rows/2);k<input_target.rows/2;k++){
					...
					for(int j=(-3*input_target.cols/2);j<3*input_target.cols/2;j+=3){
						compare[i]=compare[i] +abs(data[b_cp[i].x+j]-data2[(3*input_target.cols)/2+j]);
						compare[i]=compare[i] +abs(data[g_cp[i].x+j]-data2[(3*input_target.cols)/2+1+j]) ;
						compare[i]=compare[i] +abs(data[r_cp[i].x+j]-data2[(3*input_target.cols)/2+2+j]) ;
					}
				}
