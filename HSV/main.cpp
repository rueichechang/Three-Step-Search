...
Point supercenter(320*3,256); 
Point lefttop (-1,-1);
Point rightdown (-1,-1);
Point backgroundcenter(320*3,256);
//邊界設定
int img_top = 210;
int	img_bot = 292; 
int img_left = 260;
...
void TSS_search(Mat input_background1 ,Mat input_target1)
{
	...
	int count=0;
	//轉HSV
	Mat input_background2;
	Mat input_background;
	Mat input_background3;
	cvtColor(input_background1 ,input_background2,CV_BGR2YCrCb);
	inRange(input_background2, Scalar(0,147,77), Scalar(256,167,137),input_background);
	//morphologyEx(input_background3,input_background,MORPH_OPEN,Mat());
	//input_background=convert(input_background2);
	Mat input_target(80,100,CV_8U,Scalar(255,255,255));
	//cvtColor(input_target1,input_target  ,CV_BGR2YCbCr);
	namedWindow("ddd",WINDOW_AUTOSIZE);
	imshow("ddd",input_background);
	Point target_center((3*input_target.cols)/2 ,input_target.rows/2 );
	...
	int left_bound = 0;
	int right_bound = 3*input_background.cols-3;
	...
	for(int StepIndex=0 ; StepIndex<3 ; StepIndex++){
		    ...
		    //定義九宮格
			//b
			b_cp[0].x = supercenter.x - 3*StepSize[StepIndex];
			b_cp[1].x = supercenter.x ;
			b_cp[2].x = supercenter.x + 3*StepSize[StepIndex];

			b_cp[0].y = supercenter.y - StepSize[StepIndex];
			b_cp[1].y = supercenter.y - StepSize[StepIndex];
			b_cp[2].y = supercenter.y - StepSize[StepIndex];

			b_cp[3].x = supercenter.x - 3*StepSize[StepIndex];
			b_cp[4].x = supercenter.x ;
			b_cp[5].x = supercenter.x + 3*StepSize[StepIndex];

			b_cp[3].y = supercenter.y ;
			b_cp[4].y = supercenter.y ;
			b_cp[5].y = supercenter.y ;

			b_cp[6].x = supercenter.x - 3*StepSize[StepIndex];
			b_cp[7].x = supercenter.x ;
			b_cp[8].x = supercenter.x + 3*StepSize[StepIndex];

			b_cp[6].y = supercenter.y + StepSize[StepIndex];
			b_cp[7].y = supercenter.y + StepSize[StepIndex];
			b_cp[8].y = supercenter.y + StepSize[StepIndex];

			//g
			g_cp[0].x = supercenter.x - 3*StepSize[StepIndex] +1;
			g_cp[1].x = supercenter.x +1;
			g_cp[2].x = supercenter.x + 3*StepSize[StepIndex] +1;

			g_cp[0].y = supercenter.y - StepSize[StepIndex];
			g_cp[1].y = supercenter.y - StepSize[StepIndex];
			g_cp[2].y = supercenter.y - StepSize[StepIndex];

			g_cp[3].x = supercenter.x - 3*StepSize[StepIndex]+1;
			g_cp[4].x = supercenter.x +1;
			g_cp[5].x = supercenter.x + 3*StepSize[StepIndex]+1;

			g_cp[3].y = supercenter.y ;
			g_cp[4].y = supercenter.y ;
			g_cp[5].y = supercenter.y ;

			g_cp[6].x = supercenter.x - 3*StepSize[StepIndex]+1;
			g_cp[7].x = supercenter.x +1;
			g_cp[8].x = supercenter.x + 3*StepSize[StepIndex]+1;

			g_cp[6].y = supercenter.y + StepSize[StepIndex];
			g_cp[7].y = supercenter.y + StepSize[StepIndex];
			g_cp[8].y = supercenter.y + StepSize[StepIndex];

			//r
			r_cp[0].x = supercenter.x - 3*StepSize[StepIndex] +2;
			r_cp[1].x = supercenter.x +2;
			r_cp[2].x = supercenter.x + 3*StepSize[StepIndex] +2;

			r_cp[0].y = supercenter.y - StepSize[StepIndex];
			r_cp[1].y = supercenter.y - StepSize[StepIndex];
			r_cp[2].y = supercenter.y - StepSize[StepIndex];

			r_cp[3].x = supercenter.x - 3*StepSize[StepIndex]+2;
			r_cp[4].x = supercenter.x +2;
			r_cp[5].x = supercenter.x + 3*StepSize[StepIndex]+2;

			r_cp[3].y = supercenter.y ;
			r_cp[4].y = supercenter.y ;
			r_cp[5].y = supercenter.y ;

			r_cp[6].x = supercenter.x - 3*StepSize[StepIndex]+2;
			r_cp[7].x = supercenter.x +2;
			r_cp[8].x = supercenter.x + 3*StepSize[StepIndex]+2;

			r_cp[6].y = supercenter.y + StepSize[StepIndex];
			r_cp[7].y = supercenter.y + StepSize[StepIndex];
			r_cp[8].y = supercenter.y + StepSize[StepIndex];
			...
			for(int i=0;i<9;i++){
				...
				//背景邊界
			    if(b_cp[i].x > right_bound || b_cp[i].x < left_bound || b_cp[i].y < top_bound || b_cp[i].y > bot_bound) continue;

				if((b_cp[i].x + (3*input_target.cols)/2) > right_bound || (b_cp[i].x-(3*input_target.cols)/2) < left_bound || (b_cp[i].y-input_target.rows/2) < top_bound || (b_cp[i].y+input_target.rows/2)  > bot_bound)continue;
				
				temp++;
				for(int k=(-input_target.rows/2);k<input_target.rows/2;k++){
					uchar *data = input_background.ptr<uchar>(b_cp[i].y+k);
					uchar *data2 = input_target.ptr<uchar>(input_target.rows/2+k);

					for(int j=(-3*input_target.cols/2);j<3*input_target.cols/2;j+=3){
						compare[i]+=2*abs(data[b_cp[i].x+j]-data2[(3*input_target.cols)/2+j]);
						compare[i]+=abs(data[g_cp[i].x+j]-data2[(3*input_target.cols)/2+1+j]);
						compare[i]+=abs(data[r_cp[i].x+j]-data2[(3*input_target.cols)/2+2+j]);
					}
				}
				if(temp==1){
					...
					compare_min_i=compare[i];
					supercenter.x = b_cp[i].x; 
					supercenter.y = b_cp[i].y; 
    ...
    Point tl((supercenter.x/3) - (input_target.cols)/2 , supercenter.y - input_target.rows/2);
	Point br((supercenter.x/3) + (input_target.cols)/2 ,supercenter.y + input_target.rows/2);
...
void tracking_control()
{
	Point supercenter1((supercenter.x/3),supercenter.y);
	Point backgroundcenter1(backgroundcenter.x/3,backgroundcenter.y);
	int distance_x = supercenter1.x-backgroundcenter1.x;
	int distance_y = supercenter1.y-backgroundcenter1.y;
	...
	if(supercenter1.x>img_right)
	{		
		if(supercenter1.x>(img_right+x_interval2))
		{	
			v_cmd = -1;
		}
		else if(supercenter1.x>(img_right+x_interval1))
		{
		    v_cmd = -0.8;
		}
		else{
	...
	else if(supercenter1.x<img_left)
	{
		if(supercenter1.x<(img_right-x_interval2))
		{
		    v_cmd = 1;
	...
	if(supercenter1.y>img_bot)
	{
		if(supercenter1.y>(img_bot+y_interval2))
		{
			v_cmd2=-1;
		}
		else if(supercenter1.y>(img_bot+y_interval1))
		{
			v_cmd2=-0.8;
		}
		else
	...
	else if(supercenter1.y < img_top)
	{
...
Mat convert(Mat src)
{
	...
	for(int i=0;i<src.rows;i++)
	{
		...
		uchar *data2 = output.ptr<uchar>(i);
		for(int j=0;j<src.cols*3;j+=3)
		{
			if(data[j+1] < avgCr+range && data[j+1] > avgCr-range)
			{
				if(data[j+2] < avgCb+range && data[j+2] > avgCb-range)
				{
					data2[j]=255;
					data2[j+1]=255;
					data2[j+2]=255;
...
//void on_tracker(int,void*, Mat displayImg)
//{
//	double alpha = (double) sliderValue/100.0;
//	double beta = (1.0 - alpha);
//	imshow ("trackerbar demo",displayImg);
//
//
//}